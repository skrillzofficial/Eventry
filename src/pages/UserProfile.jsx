...
const UserProfile = () => {
  ...
  const loadBookingsData = async () => {
    try {
      ...
      const bookings = bookingsResult.success ? (bookingsResult.data.data || bookingsResult.data.bookings || []) : [];
      
      const today = new Date();
      
      // Filter attended events (past events)
      const attended = bookings.filter(booking => 
        booking.event && new Date(booking.event.date || booking.event.startDate) < today
      ).map(b => b.event);
  ...
  const loadTransactionsData = async () => {
    try {
      ...
      const transactions = transactionsResult.success ? 
        (transactionsResult.data.data || transactionsResult.data.transactions || []) : [];
      ...
      const totalSpent = successfulTransactions.reduce((sum, t) => sum + (t.amount || 0), 0);
  ...
  const generateActivityFromBookings = (bookings) => {
    ...
    if (sortedBookings.length > 0) {
      ...
      activities.push({
        ...
        description: recent.event?.title || recent.event?.name || "Event Ticket",
    ...
    const attended = sortedBookings.filter(b => 
      b.event && new Date(b.event.date || b.event.startDate) < new Date()
    );
    if (attended.length > 0) {
      activities.push({
        ...
        time: formatTimeAgo(attended[0].event?.date || attended[0].event?.startDate),
        type: "attendance"
      });
    }
    ...
    if (upcoming.length > 0) {
      activities.push({
        ...
        description: `${upcoming[0].event?.title || upcoming[0].event?.name} is coming soon`,
        time: formatTimeAgo(upcoming[0].event?.date || upcoming[0].event?.startDate),
        type: "reminder"
      });
    }
...
const StatCard = ({ title, value, icon: Icon, iconColor }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 hover:shadow-lg transition-all group">
    <div className="flex items-center justify-between">
      ...
      <div className={`p-3 ${iconColor} rounded-lg group-hover:scale-110 transition-transform`}>
...
const QuickActionsSection = () => (
  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <Link to="/discover" className="flex items-center p-4 border border-gray-200 rounded-lg hover:border-[#FF6B35] hover:bg-[#FF6B35]/5 transition-all group">
      ...
      <Link to="/my-tickets" className="flex items-center p-4 border border-gray-200 rounded-lg hover:border-[#FF6B35] hover:bg-[#FF6B35]/5 transition-all group">
...
const ActivitySection = ({ activities }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
    <div className="flex items-center justify-between mb-4">
      ...
      <Link to="/activity" className="text-sm text-[#FF6B35] hover:text-[#FF8535] flex items-center group">
    ...
    <div className="space-y-3">
      {activities.length > 0 ? (
        ...
        <div className="text-center py-8 text-gray-500">
          ...
          <Link to="/discover" className="text-[#FF6B35] hover:text-[#FF8535] text-sm mt-2 inline-block">
...
const EventHistorySection = ({ events }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
    <div className="flex items-center justify-between mb-4">
      ...
      <Link to="/my-events" className="text-sm text-[#FF6B35] hover:text-[#FF8535] flex items-center group">
        View all <ArrowRight className="h-4 w-4 ml-1 group-hover:translate-x-1 transition-transform" />
      </Link>
    </div>
    <div className="space-y-3">
...
const UpcomingBookingsSection = ({ bookings }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">My Upcoming Events</h3>
      </div>
      <div className="space-y-3">
